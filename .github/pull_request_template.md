## Summary

- 

## Change type

- [ ] Type profile change
- [ ] Overlay change
- [ ] Runtime SKILL.md change
- [ ] Documentation/example change
- [ ] Repository tooling / CI change

## Profiles or overlays affected

List exact files, for example `references/INTP.md` or `overlays/rushed.md`.

## Cognitive-function rationale

Explain why the change fits the relevant function stack or overlay behavior. Use observable coding behavior, not stereotypes.

## Test plan

- [ ] Ran `python scripts/validate_repo.py`
- [ ] Tested at least one planning prompt, if behavior changed
- [ ] Tested at least one debugging prompt, if behavior changed
- [ ] Tested an explicit user override, if type behavior changed

## Notes for reviewers

Call out tradeoffs, uncertain areas, or follow-up work.
